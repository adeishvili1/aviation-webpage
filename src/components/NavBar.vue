  ...
  <div class="fixed top-0 left-0 w-full z-50">
      ...
      <div class="w-full px-10 py-1.5 flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="flex items-center gap-1 text-sm">
            ...
            <button @click="setLocale('en')" class="px-2 py-0.5 rounded transition-colors" :class="locale === 'en' ? 'text-white font-bold bg-white/10' : 'text-white/50 hover:text-white'">Eng</button>
            <span class="text-white/20">|</span>
            <button @click="setLocale('ru')" class="px-2 py-0.5 rounded transition-colors" :class="locale === 'ru' ? 'text-white font-bold bg-white/10' : 'text-white/50 hover:text-white'">Rus</button>
          </div>
        </div>
      </div>
    </div>
    ...
    >
      <div class="w-full px-10 py-3 flex items-center justify-between">
        <!-- Logo -->
        <router-link to="/" class="group cursor-pointer shrink-0">
          <!-- Old logo fallback: <img class="h-12 w-auto object-contain" src="../assets/logo1.png" alt="logo"> -->
          <img class="h-14 lg:h-15 w-auto max-w-[320px] object-contain" src="../assets/tam-navbar-footer-logo.png" alt="JSC TAM logo">
          <div class="hidden">
          ...
          <template v-for="link in navLinks" :key="link.id">
            ...
            <div v-else class="relative">
              ...
              >
                ...
                >
                  <div class="py-2">
                    <template v-for="sub in link.children" :key="sub.key">
                      <!-- Regular item -->
                      <router-link
                        v-if="!sub.nested"
                        :to="sub.to"
                      ...
                      <div v-else>
                        <div class="flex items-center gap-3 px-4 pt-3 pb-1">
                          ...
                          <span class="text-sm font-semibold text-white/90">{{ $t(sub.key) }}</span>
                        </div>
                        <router-link
                        ...
                        >
                          ...
                          <div>
                            ...
                            <div v-if="nested.descKey" class="text-xs text-white/40 mt-0.5">{{ $t(nested.descKey) }}</div>
                          </div>
                        </router-link>
                      </div>
                    </template>
          ...
          <div class="px-6 py-4 space-y-1">
            ...
            <div class="pt-4 mt-4 border-t border-blue-500/10">
              ...
              <div class="flex items-center gap-2 text-sm">
                <button @click="setLocale('ka')" class="px-3 py-1 rounded transition-colors" :class="locale === 'ka' ? 'text-white font-bold bg-white/10' : 'text-white/50'">ქარ</button>
                <button @click="setLocale('en')" class="px-3 py-1 rounded transition-colors" :class="locale === 'en' ? 'text-white font-bold bg-white/10' : 'text-white/50'">Eng</button>
                <button @click="setLocale('ru')" class="px-3 py-1 rounded transition-colors" :class="locale === 'ru' ? 'text-white font-bold bg-white/10' : 'text-white/50'">Rus</button>
              </div>
            </div>
          </div>
        </div>
